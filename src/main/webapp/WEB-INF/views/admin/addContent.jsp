...
<form method="post">
    <input type="hidden" id="getContentId" name="contentId" value="${contentModel.id}"/>
    <div class="control-group" id="titleId">
        <label class="control-label" for="inputTitle">标题</label>
        <div class="controls">
            <input class="input-large span4" type="text" name="title" id="inputTitle" placeholder="标题">
        </div>
    </div>
    ...
    $(function(){
        ...
        $("select").change(function () {
            ...
            if (str == "企业相册"){
                $("#areaDiv").hide();
                $("#uploaderDiv").show();
                $("form").attr("action","${ctx}/admin/add/image/${typeName}");
            } else {
                $("#areaDiv").show();
                $("#uploaderDiv").hide();
                $("form").attr("action","${ctx}/admin/add/${typeName}");
            }

            if (type_name == "market" || type_name == "news" || str=="招聘信息"){
                $("#uploaderDiv").show();
            }
